...
dp_uv_lookup_256_np=np.load('densepose_data/dp_uv_lookup_256_new.npy')

def getSymXYcoordinates(iuv,resolution = 256):
    xy, xyMask = getXYcoor(iuv, resolution = resolution)
    #f_xy, f_xyMask = getXYcoor(flip_iuv(np.copy(iuv)),dp_uv_lookup_256_np, resolution = resolution)
    ...
    combined_mask = np.clip(xyMask, a_min=0, a_max=1)#+f_xyMask
    f_xyMask=0
...
def mapper(iuv, resolution=256):
    #dp_uv_lookup_256_np = np.load('dp_uv_lookup_256.npy')
    H, W, _ = iuv.shape
    iuv_raw = iuv[iuv[:, :, 0] > 0]
    #print(np.max(iuv[:,:,2]))
...
def dp2coor(dp, resolution = 256):
#for i in range(1987, len(images)):
    #im_name = images[i]
    #print ('%d/%d'%(i+1, len(images)))

    #dp = os.path.join(args.dp_path, im_name.split('.')[0]+'.png')

    iuv_copy = np.array(dp)
    iuv = iuv_copy[...,::-1].copy() 
    h, w, _ = iuv.shape
    if np.sum(iuv[:,:,0]==0)>=(h*w)-50:
        print ('no human: invalid image')
        uv_coor=np.zeros((iuv.shape[0],iuv.shape[1],2))
        uv_mask=np.zeros((iuv.shape[0],iuv.shape[1]))
        uv_symm_mask=np.zeros((iuv.shape[0],iuv.shape[1]))
        return uv_coor, uv_mask, uv_symm_mask
    else:
        uv_coor, uv_mask, uv_symm_mask  = getSymXYcoordinates(iuv, resolution = resolution)
        return uv_coor, uv_mask, uv_symm_mask
        ...
        #np.save(os.path.join(args.save_path, '%s_uv_mask.npy'%(im_name.split('.')[0])), uv_mask)
        #np.save(os.path.join(args.save_path, '%s_uv_symm_mask.npy'%(im_name.split('.')[0])), uv_symm_mask)
